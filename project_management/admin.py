...
class ProjectManagementAdmin(admin.ModelAdmin):
    ...
    list_display = [
        'project_id', 'project_name', 'description_preview', 'formatted_start_date', 'formatted_end_date',
    ...
    def action_buttons(self, obj):
        edit_url = f'/admin/project_management/projectmanagement/{obj.pk}/change/'
        delete_url = f'/admin/project_management/projectmanagement/{obj.pk}/delete/'
    ...
    def get_view_details_url(self, obj):
        return f"/admin/project_management/projectmanagement/{obj.pk}/donation_details/"
    ...
    def get_urls(self):
        ...
        custom_urls = [
            path(
                '<int:project_id>/donation_details/',
                self.admin_site.admin_view(self.donation_details_view),
                name='project_donation_details',
            ),
        ]
        return custom_urls + urls

    def donation_details_view(self, request, project_id):
        project = get_object_or_404(ProjectManagement, pk=project_id)
        donations = OrderManagement.objects.filter(project=project, payment_status='completed')
        ...
        return render(request, 'admin/project_management/donation_details.html', context)